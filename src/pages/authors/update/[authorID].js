...
export default function UpdateBook({ returnProps }) {
    ...
    const saveAuthor = async (data) => {
        console.log(`${API_URL}/authors/${author[0].authorID}`);
        const response = await fetch(`/api/authors/${author[0].authorID}`, {
            method: "PUT",
            mode: "cors",
            cache: "no-cache",
            headers: {
            ...
            {/* <form onSubmit = {handleSubmit(saveAuthor)}>
                ...
                <div style = {{ display: "flex", flexDirection: "row", gap: "3rem", width: "90vw" }}>
                    <div>
                        ...
                        <input class = "form-control text-input" id = "firstName" name = "firstName" {...register('firstName', {required: 'This field is required'})} placeholder = "First Name" defaultValue = {author[0].firstName} required/>
                        <label htmlFor = "lastName">Last Name</label><br/>
                        <input class = "form-control text-input" id = "lastName" name = "lastName" {...register('lastName', {required: 'This field is required'})} placeholder = "Last Name" defaultValue = {author[0].lastName} required/>
                    </div>

                    <div>
                        
                        <label htmlFor="publisher">Publisher</label><br />
                        <input id="publisher" {...register("publisher")} placeholder="Name of Publisher" class="form-control" defaultValue = {author[0].publisher}/>

                        <label htmlFor="authorID">Author ID</label><br />
                        <input type = "text" id="authorID" {...register("authorID")} placeholder="Author ID" class="form-control" defaultValue = {author[0].authorID}/>
